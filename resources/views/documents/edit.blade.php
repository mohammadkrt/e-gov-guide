            ...
            <div class="card" >
                <div class="card-body">
                    <div class="card-header card-header-primary">
                        <h4 class="card-title ">تعديل مستند</h4>
                        ...
                        <form action="{{route('document.update', $document->id)}}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control" id="service_id" name="service_id">
                                        ...
                                        @foreach($services as $service)
                                            <option value="{{ $service->id }}" {{$document->service_id == $service->id  ? 'selected' : ''}}>{{ $service->service_name}}</option>
                                        @endforeach
                                    </select>
                                    @error('service_id')
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="document_name" value="{{$document->document_name}}" class="form-control" id="document_name" placeholder="إســــم المستند">
                                    @error('document_name')
                                    <small class="form-text text-danger">{{$message}}</small>
                                    @enderror
                                </div>
