                        ...
                        <form action="{{route('unit.update', $unit->id)}}" method="post">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="unit_name" value="{{$unit->unit_name}}" class="form-control" id="unit_name" placeholder="إســــم الوحدة">
                                    @error('unit_name')
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control" id="state_id" name="state_id">
                                        ...
                                        @foreach($states as $state)
                                            <option value="{{ $state->id }}" {{$unit->state_id == $state->id  ? 'selected' : ''}}>{{ $state->state_name}}</option>
                                        @endforeach
                                    </select>
                                    @error('state_id')
                                    <small class="form-text text-danger">{{$message}}</small>
